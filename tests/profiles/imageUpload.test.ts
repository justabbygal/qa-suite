 ...
 *
 * No network calls are made; all storage interactions are pure-function tests.
 */
...
describe('validateImageFile', () => {
  ...
  describe('rejected MIME types', () => {
    ...
    it('error message lists all allowed types', () => {
  ...
  describe('type check precedes size check', () => {
    it('returns a type error (not a size error) when both are invalid', () => {
      const file = makeFile('big.pdf', 'application/pdf', 10 * ONE_MB);
      const result = validateImageFile(file);
      expect(result.valid).toBe(false);
      // The error should reference the MIME type, not the size
...
describe('isAllowedImageType', () => {
  ...
  it('is case-sensitive (uppercase type rejected)', () => {
...
describe('getFileExtension', () => {
  ...
  it.each(cases)('maps %s → %s', (mimeType, ext) => {
...
describe('generateAvatarPath', () => {
  ...
  it('uses the correct extension for png', () => {
    const path = generateAvatarPath(ORG_ID, USER_ID, 'image/png', TIMESTAMP);
    expect(path.endsWith('.png')).toBe(true);
  });

  it('uses the correct extension for webp', () => {
    const path = generateAvatarPath(ORG_ID, USER_ID, 'image/webp', TIMESTAMP);
    expect(path.endsWith('.webp')).toBe(true);
  });

  it('uses the correct extension for gif', () => {
    const path = generateAvatarPath(ORG_ID, USER_ID, 'image/gif', TIMESTAMP);
    expect(path.endsWith('.gif')).toBe(true);
  });

  it('produces a deterministic path given the same timestamp', () => {
  ...
  it('produces unique paths for different users', () => {
    const path1 = generateAvatarPath(ORG_ID, 'user-aaa', 'image/jpeg', TIMESTAMP);
    const path2 = generateAvatarPath(ORG_ID, 'user-bbb', 'image/jpeg', TIMESTAMP);
    expect(path1).not.toBe(path2);
  });

  it('produces unique paths for different organisations', () => {
    const path1 = generateAvatarPath('org-aaa', USER_ID, 'image/jpeg', TIMESTAMP);
    const path2 = generateAvatarPath('org-bbb', USER_ID, 'image/jpeg', TIMESTAMP);
    expect(path1).not.toBe(path2);
  });

  it('uses a live timestamp when no timestamp argument is provided', () => {
    const before = Date.now();
    const path = generateAvatarPath(ORG_ID, USER_ID, 'image/jpeg');
    const after = Date.now();

    // Extract the numeric timestamp from the path
...
describe('formatFileSize', () => {
  ...
  it('formats 1 048 576 bytes as MB', () => expect(formatFileSize(ONE_MB)).toBe('1.0 MB'));
...
describe('validate → generate path workflow', () => {
  it('generates a path only when validation passes', () => {
    const validFile = makeFile('avatar.png', 'image/png', ONE_MB);
    const result = validateImageFile(validFile);
    expect(result.valid).toBe(true);

    if (result.valid) {
      const path = generateAvatarPath('org-1', 'user-1', validFile.type, 123456789);
      expect(path).toBe('org-1/user-1/avatar-123456789.png');
    }
  });

  it('does not generate a path when validation fails', () => {
    const invalidFile = makeFile('virus.exe', 'application/octet-stream', ONE_MB);
    const result = validateImageFile(invalidFile);
    expect(result.valid).toBe(false);
    // No path should be generated for invalid files
    expect(result.error).toBeDefined();
  });
});
