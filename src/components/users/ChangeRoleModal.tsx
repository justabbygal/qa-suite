...
/** Convert the lowercase UIRole to the capitalized API value. */
...
export function ChangeRoleModal({
  ...
  // All three roles in display order
  ...
  function disabledReason(role: UIRole): string {
    if (!canManage) {
      return `You cannot manage a ${ROLE_DISPLAY[targetUser.role].label}.`;
    }
    if (!assignableRoles.includes(role)) {
      return `Your role does not allow assigning ${ROLE_DISPLAY[role].label}.`;
    }
    return "";
  }
  ...
  async function handleConfirm() {
    ...
    try {
      await changeUserRole({
        ...
        organizationId,
        newRole: toApiRole(selectedRole),
      });
      ...
      <AlertDialog open={open} onOpenChange={(isOpen) => { if (!isOpen) handleCancel(); }}>
        <AlertDialogContent aria-describedby="change-role-description">
          ...
          {!canManage && (
            ...
            >
              You do not have permission to change the role of a{" "}
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={handleCancel} autoFocus>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
  ...
  const impact = selectedRole
    ? getRoleChangeImpact(targetUser.role, selectedRole)
    : "";
