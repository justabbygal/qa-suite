 ...
 * Public URLs follow: /storage/v1/object/public/<bucket>/<path>
 */
...
// ---------------------------------------------------------------------------
// Old-image cleanup (best-effort — does not fail the request on error)
// ---------------------------------------------------------------------------
...
async function deleteOldAvatars(
  ...
  // Derive thumbnail path from main path filename if it matches the naming convention.
  // e.g. "org/user/avatar-1700000000000.jpg" → "org/user/avatar-thumb-1700000000000.webp"
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  let thumbnailUrl = avatarUrl; // fallback to main image if no thumbnail
  ...
  if (updateErr) {
    // Files were uploaded but DB update failed — remove the new files, then error
    await supabase.storage
      .from(AVATAR_BUCKET)
