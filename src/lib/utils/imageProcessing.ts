 ...
 *   4. Upload the returned `main` and `thumbnail` blobs to POST /api/profiles/upload-image.
 *      For files > 1 MB, pass an `onProgress` callback to show a progress indicator.
 */
...
export interface ProcessedImage {
  /** Resized and compressed main image, max 400 x 400 px. */
  main: Blob;
  /** Square center-cropped thumbnail, 80 x 80 px. */
  thumbnail: Blob;
...
/**
 * Resizes an image to fit within maxDimension x maxDimension pixels while
 ...
 *   1. Resizes the main image to fit within 400 x 400 px.
 *   2. Generates an 80 x 80 square thumbnail via center crop.
 *
...
/**
 * Uploads a FormData payload using XMLHttpRequest to support progress events.
