...
describe('Integration – update workflow', () => {
  ...
  it('enforces permission constraints during update (clears settingsAccess when featureAccess is removed)', async () => {
    const existing = makeRegisteredModule({
      permissions: makeRolePermissions({
...
describe('Integration – error type contract', () => {
  it('all thrown errors are instances of ModuleServiceError', async () => {
    const scenarios: Array<{ name: string; fn: () => Promise<unknown> }> = [
      {
        name: 'registerModule VALIDATION_ERROR',
        fn: () =>
          registerModule(
            buildSequentialSupabase([]),
            makeModuleManifest({ module: 'Bad Module' }),
            'org'
          ),
      },
      {
        name: 'registerModule DUPLICATE',
        fn: () =>
          registerModule(
            buildSequentialSupabase([{ data: toDbRow(makeRegisteredModule()), error: null }]),
            makeModuleManifest(),
            'org'
          ),
      },
      {
        name: 'updateModule NOT_FOUND',
        fn: () =>
          updateModule(
            buildSequentialSupabase([{ data: null, error: { message: 'not found' } }]),
            'ghost',
            { displayName: 'x' }
          ),
      },
      {
        name: 'deregisterModule DATABASE_ERROR',
        fn: () =>
          deregisterModule(
            buildSequentialSupabase([{ data: null, error: { message: 'fk' } }]),
            'id'
          ),
      },
    ];
    ...
    for (const { name, fn } of scenarios) {
      await expect(fn()).rejects.toBeInstanceOf(ModuleServiceError);
    }
  });
});
...
describe('Integration – permission generation', () => {
  it('generated permissions always pass validatePermissions', async () => {
    ...
    for (const manifest of manifests) {
      ...
      // We just want to verify no validation errors are thrown
