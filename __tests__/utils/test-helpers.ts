...
export function buildErrorResponse(
  ...
  return {
    ...
    error,
    message:
      message ?? defaultMessages[error] ?? "An unexpected error occurred.",
  };
}
...
export function mockPendingFetch(): () => void {
  ...
  return () => resolve(makeFetchResponse(200, buildSuccessResponse()));
}
...
/**
 * Parses the body sent to the most recent fetch call and returns it as a plain
